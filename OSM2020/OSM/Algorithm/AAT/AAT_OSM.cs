...
namespace OSM2020.OSM.Algorithm.AAT
{
    class AAT_OSM : OSMBase
    {
        ...
        {
            this.Candidates = new Dictionary<Agent, Candidate>(); //agentと重み候補集合を組み合わせた辞書
            foreach (var agent in this.MyAgentNetwork.Agents) //agent一人一人に対して
            {
                var can = new Candidate(agent, opinion_share_num,this.AwaRateWindowSize); //重み候補集合作成
        ...
        protected virtual void SelectionWeight() //重み候補集合の現在或いは前後のどれかから選ぶ．
        {
            foreach (var candidate in this.Candidates) //全てのエージェントとその重み候補集合について
            {
                ...
                candidate.Key.SetCommonWeight(candidate.Value.GetSelectCanWeight()); //エージェントの周りのリンクの重みを更新
            }
        }
